...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/23 23:55:04 by mrony             #+#    #+#             */
/*   Updated: 2023/02/24 13:01:35 by mrony            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_resolution(t_stack *a, t_info *data, t_list *ops)
{
	t_stack	*b;
...
int	main(int ac, char **av)
{
	...
	if (!ops && is_sorted(&a) == 0)
		return (ft_stackclear(&a), ft_lstclear(&ops, free),
			ft_printf("KO\n"), 0);
	if (!ops && is_sorted(&a) == 1)
		return (ft_stackclear(&a), ft_lstclear(&ops, free),
			ft_printf("OK\n"), 0);
	if (ft_resolution(a, &data, ops) == 1)
		return (ft_stackclear(&a), ft_lstclear(&ops, free),
			write(2, "Error\n", 6), 1);
	return (0);
}
